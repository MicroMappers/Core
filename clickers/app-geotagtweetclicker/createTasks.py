...
def task_formatter(app_config,app, headerinfo, row, n_answers):
    ...
    if len(row) > 3 :
        info = dict(question=app_config['question'],
                    ...
                    text = row_value_in_line(headerinfo, 'text', row),
                    date = row_value_in_line(headerinfo, 'date', row),
                    username = row_value_in_line(headerinfo, 'username', row),
                    userid = row_value_in_line(headerinfo, 'userid', row),
                    )
        print info
