...
def imageURL_extractor(url) :
    ...
    if len(url) > 0 :
        try:
            ...
            #imgurl = soup.find_all("a", attrs={"class": "twitter-timeline-link media-thumbnail"})
            if len(imgurl[0]) > 0:
                imgurlText = imgurl[0].attrs
                imgurl = imgurlText['data-url']
                #u'data-url' (4400983568)
...
def task_formatter(app_config,app,headerinfo, row, n_answers):
    ...
    :rtype: integer
    """
    #TWEETID,TWEET,TIMESTAMP,AUTHOR,LAT,LONG,url
    ...
    # row[3] = TWEET
    # row[4] = TIMESTAMP
    # FILTER,USERNAME,URL,TWEET,TIMESTAMP
   # url = row_value_in_line(headerinfo, 'TWEET', row)
    list = []
    if len(row) > 0 :
        if len(list) > 0 :
            if row[2] in list:
...
if __name__ == "__main__":
    ...
    if options.create_app:
        ...
        with open('tornadotweets_filtered3.csv', 'rb') as csvfile:
